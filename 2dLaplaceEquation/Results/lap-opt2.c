...
float laplace_step_error ( float *in, float *out, int n, int m )
{
  int i, j;
  float error=0.0f;
  for ( i=1; i < m-1; i++ )
    for ( j=1; j < n-1; j++ ){
      out[i*n+j]= stencil(in[i*n+j+1], in[i*n+j-1], in[(i-1)*n+j], in[(i+1)*n+j]);
      error = fmaxf( error, sqrtf( fabsf( in[i*n+j] - out[i*n+j] )));
  }
  return error;
...
int main(int argc, char** argv)
{
  ...
  {
    iter++;
    error= laplace_step_error (A, Anew, n, m);
