...
void recalculatePosition(real3 * force, real4 * in, real3 * vel, real dt)
{
               ...
               real    dt,  int n)
{
  ...
  for (i = 0; i < n; i++)
  {
    ...
    for (j = i+1; j < n; j++) {
      ...
      force[i].x += ff.x;  force[i].y += ff.y;  force[i].z += ff.z;
      force[j].x += -ff.x;  force[j].y += -ff.y;  force[j].z += -ff.z;
    }
    recalculatePosition(&force[i], &in[i], &vel[i], dt);
  }
}
