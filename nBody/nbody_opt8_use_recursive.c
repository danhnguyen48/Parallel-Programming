...
void calculateBody(int, int, real3 *, real4 *, real4 *, real3 *, real);
real3 calculateInteraction(int, int, real4 *, real3 *, int);
...
void integrate(real4 * out, real4 * in,
               ...
               real    dt,  int n)
{
  memset(force, 0.0, n*sizeof(real3));
  calculateBody(0, n, force, in, out, vel, dt);
}
...
void calculateBody(int bodyIndex, int n, real3 * force, real4 * in, real4 * out, real3 * vel, real dt) {
    if (bodyIndex == n) return;
    real3 currentBodyInteraction = calculateInteraction(bodyIndex, bodyIndex + 1, in, force, n);
    force[bodyIndex].x += currentBodyInteraction.x;
    force[bodyIndex].y += currentBodyInteraction.y;
    force[bodyIndex].z += currentBodyInteraction.z;
    recalculatePosition(force[bodyIndex], &in[bodyIndex], &out[bodyIndex], &vel[bodyIndex], dt);
    calculateBody(bodyIndex + 1, n, force, in, out, vel, dt);
}

real3 calculateInteraction(int bodyIndex, int nextBody, real4 * in, real3 * force, int n) {
    if (nextBody > n-1) {
        real3 t = {0,0,0};
        return t;
    }
    real3 ff = bodyBodyInteraction(in[bodyIndex], in[nextBody]);
    force[nextBody].x += -ff.x;   force[nextBody].y += -ff.y;   force[nextBody].z += -ff.z;
    if (nextBody == n-1) return ff;
    real3 followingFF = calculateInteraction(bodyIndex, nextBody + 1, in, force, n);
    real3 sum = {ff.x + followingFF.x, ff.y + followingFF.y, ff.z + followingFF.z};
    return sum;
}