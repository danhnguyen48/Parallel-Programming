...
#define SOFTENING_SQUARED3 (SOFTENING_SQUARED*SOFTENING_SQUARED*SOFTENING_SQUARED) 
...
real3 bodyBodyInteraction(real4 iPos, real4 jPos)
{
  ...
  s= distSqr*distSqr*distSqr;
  if (s < SOFTENING_SQUARED3) s = sqrtf(SOFTENING_SQUARED3);
  else s = sqrtf(s);
  s = jPos.w / s;
