...
float laplace_step ( float *M, int n, int m, bool left )
{
    float error = 0.0f;
    int index;
        ...
        for ( int j=2; j < 2*n-2; j+=2 )
            {
                ...
                *(M + i*n*2 + j + 1 - index) = stencil(*(M + (i-1)*n*2 + j + index), 
                                                     ...
                                                     *(M + i*n*2 + j - 2 + index));
                // Calculate the error 
                error = fmaxf( error, fabsf( *(M + i*n*2 + j) - *(M + i*n*2 + j + 1) ));
            }
...
void laplace_init ( float *M, int n, int m )
{
    ...
    memset(M, 0, n*2*m*sizeof(float));
    for (int i=0; i<n*2; i++) {
        *(M + i) = 0.f; // top boundary
        *(M + 2*n*(m-1) + i) = 0.f; // bottom boundary
    } 
    for (int j=0; j<m; j++) {
        *(M + j*2*n)  = sinf(pi*j / (n-1)); // left boundary
        *(M + j*2*n + 1)  = sinf(pi*j / (n-1)); // left boundary for temp element
        *(M + j*2*n + 2*n - 2) = sinf(pi*j / (n-1))*expf(-pi); // right boundary
        *(M + j*2*n + 2*n - 2 + 1) = sinf(pi*j / (n-1))*expf(-pi); // right boundary for temp element
    }
}
...
int main(int argc, char** argv)
{
    ...
    A    = (float*) malloc( n*m*2*sizeof(float) );
